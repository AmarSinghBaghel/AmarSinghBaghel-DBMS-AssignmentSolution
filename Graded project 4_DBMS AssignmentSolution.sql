...
select /*p1.Passenger_Gender,*/ 
 case
     when p1.Passenger_Gender = 'F' Then 'Female'
     when p1.Passenger_Gender = 'M' Then 'male'
     else 'Supplier should not be considered'
end As  Passenger_Gender  ,p1.Passenger_count as 'Count of Passenger travelled for a >= 600 KM s' from 
(SELECT Gender as Passenger_Gender,COUNT(Gender) as Passenger_count  FROM passenger  WHERE Distance >=600 and gender= 'F'
UNION
SELECT Gender as Passenger_Gender ,COUNT(Gender) as Passenger_count FROM passenger  WHERE Distance >=600 and gender= 'M') as p1;
...
/*ANSWER 7*/
SELECT p1.name as Passenger_name ,p1.d as 'distance < 1000 KM s',price.price as price
...
SELECT Passenger_name,Distance*100/(select sum(Distance) as s from passenger) As 'Percentage of Distance Travelled (in %)'  
FROM passenger;
